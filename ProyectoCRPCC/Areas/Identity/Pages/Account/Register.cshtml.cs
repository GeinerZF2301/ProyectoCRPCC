...
namespace ProyectoCRPCC.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            _roleManager.Roles.Where(r => r.Name !=
                DS.Role_Cliente).Select(n => n.Name).Select(l => new SelectListItem
                {
                    Text = l,
                    Value = l
                })
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var existe = _userManager.Users.FirstOrDefault(u => u.UserName == Input.UserName);
                if (existe != null)
                {
                    TempData["msg"] = "<script language='javascript'>Swal.fire('Este usuario ya existe')" + "</script>";
                    return Page();
                }
                else
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        if (!await _roleManager.RoleExistsAsync(DS.Role_Registrador))
                        {
                            await _roleManager.CreateAsync(new IdentityRole(DS.Role_Registrador));
                        }
